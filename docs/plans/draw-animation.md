...
- [x] WP-007: 연출 리듬 재설계 — "주인공 교체" (2026-07-04 사용자 결정)
  - 목적: 믹싱 중심 연출의 반복 실패(혼돈/기계적)를 구조적으로 해결 — 드라마를 공개 순간으로.
    믹싱은 차분한 배경(공 18개·큼직), 공개마다 튜브 슛→중앙 대형 컷인(금색 플래시)→트레이
    안착, 마지막 공 앞 서스펜스(드럼 떨림·요동 3배).
  - 변경 파일: DrawOverlay.tsx(phase 재편: mixing→[shooting→showcase]×6+suspense→result,
    shotCount/settledCount 분리), DrawMachineCanvas.tsx(튜브·서스펜스 떨림·차분한 믹싱·공 18개),
    styles.css(.draw-cutin), DrawOverlay.test.tsx(새 리듬으로 갱신), spec 요구사항 10(약 7~9초),
    design 핵심 결정 4·비주얼·변경 이력
  - 완료 조건: spec 완료 조건 전체 유지(순차 공개·뽑힌 순서·결과 컷·건너뛰기·번호 불변·접근성).
  - 검증: 테스트 29개 통과(리듬 테스트 갱신 포함), lint·typecheck·build 통과, 브라우저 육안
    (컷인 등장·안착·서스펜스·결과 컷=용지 일치). Codex 재검토(로직 변경) — 커밋 후 실행.
  - 위험: 연출 총 길이 7.8초가 다시 "느리다" 피드백을 받을 수 있음(타이밍 상수로 즉시 조정 가능).
  - 상태: done (2026-07-04)
...
- [x] spec 완료 조건 충족 (2026-07-04 — 연출 흐름·번호 불변·건너뛰기·연속뽑기·접근성·표현 전수 확인)
- [x] 테스트 (정상·오류·경계값 — 29개 통과)
- [x] lint/typecheck (통과)
- [x] Codex 독립 검토 (3회: 최초 특이사항 없음 / 물리 교체 후 [권장] 1건 반영 / 리듬 재설계 후 특이사항 없음)
- [x] CI 통과 (모든 push에서 success)
- [x] 사람 최종 확인 → main 병합 (2026-07-04 사용자 위임 — "권한줄테니깐 전부 다 마무리해줘")
...
- 2026-07-03: 사용자 실사용 피드백 반영 — ① 연출 단축(MIX 2500→1400, REVEAL 1200→650 ≈ 6초,
  spec 요구사항 10 함께 갱신), ② 오버레이 머신 확대(base 340 × scale 1.25) + 믹싱 turbo 요동
  (nth-child로 궤적·주기·위상 분산, 요동 ≤26px로 구 안 유지), ③ 홈페이지 전반 사이즈업
  (타이틀·버튼·칩·용지 640/공 46·통계 공 44·wrap 1120 등 — 스타일만, 동작 무변경).
  머신 구 280→340·공 48→52로 좌표 재계산(LottoMachine). 게이트 재통과·브라우저 재확인.
- 2026-07-03: 2차 피드백 — 오버레이 머신 1.25→1.45배(+margin-top 180), 연출 총 6초→약 5초
  (MIX 1200·REVEAL 550, spec 5~7초 범위 내라 spec 무변경), turbo 가속·증폭(0.55~0.85s,
  요동 ≤32px=안전 한계)·통 진동(machine-rumble) 추가, 오버레이 공 62px. BGM·효과음은
  다음 기능으로 PROJECT.md에 기록(범위 밖 유지).
